/*
  ...
  * Last committed			:  $Revision: 1543 $
  * Revision of last commit	:  $Rev: 1543 $
  * Date of last commit     :  $Date: 2016-01-26 15:46:39 +0100 (Tue, 26 Jan 2016) $ 
  *
  ******************************************************************************
  * @attention
  *
...
public class stnfcndefhandler {
	...
	public void setNdefAar(String Aar, int recordId)
	{
		...
		try {
			...
			Log.d(TAG,"Unalbe to convert external type in byte UTF8");
			e.printStackTrace();
		}
	...
	public void setNdefVCard(String VcardPayload, int recordId )
	{
		...
		try {
			...
			Log.d(TAG,"Unalbe to convert Vcard type in byte UTF8");
			e.printStackTrace();
		}
		this.mstnfcndefrecord[recordId].setMtype(textType);
	    this.mstnfcndefrecord[recordId].setmIDLength(0);
	...
	public void setNdefBTHandover(byte [] payload, int recordId)
	{
		...
		try {
			...
			Log.d(TAG,"Unalbe to convert BT type in byte UTF8");
			e.printStackTrace();
		}
		this.mstnfcndefrecord[recordId].setMtype(textType);
	    this.mstnfcndefrecord[recordId].setmIDLength(0);
	...
	public void setNdefWiFiHandover(byte [] payload, int recordId)
	{
		...
		try {
			...
			Log.d(TAG,"Unalbe to convert Wifi type in byte UTF8");
			e.printStackTrace();
		}
		this.mstnfcndefrecord[recordId].setMtype(textType);
	    this.mstnfcndefrecord[recordId].setmIDLength(0);
	...
	public ndefError parseNdefBuffer(short offset)
	{
		...
		else
		{
			...
			Log.v(this.getClass().getName(), "parseNdefBuffer error: malformed NDEF HEADER");
